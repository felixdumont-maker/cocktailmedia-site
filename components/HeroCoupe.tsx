...
/*
 * SVG paths du logo officiel Cocktail Média — coupe de martini
 * viewBox: 0 0 626.13 876
 *
 * GLASS_PATHS : structure du verre (#2b2b2b) — tige, base, bol, garnitures
 * SPLASH_PATHS: éclaboussure officielle (#e83b14) — révélée à la fin de l'anim
 *
 * Polygone bowl (intérieur du bol de la coupe uniquement) :
 *   top-left (60, 205) · top-right (566, 205) · bot-right (366, 505) · bot-left (260, 505)
 * = le trapèze couvrant l'intérieur du bol, excluant tige et base
 */
...
// Coordonnées du polygone bowl intérieur (dans l'espace SVG 626×876)
// Exclut volontairement la tige et la base — seul l'intérieur du bol se remplit
const BOWL_TOP    = 205  // y du bord du bol
const BOWL_BOTTOM = 505  // y du bas du bol / jonction tige
const BOWL_LEFT_T = 60   // x bord gauche au niveau du bord
const BOWL_RIGHT_T = 566 // x bord droit au niveau du bord
const BOWL_LEFT_B  = 260 // x bord gauche en bas
const BOWL_RIGHT_B = 366 // x bord droit en bas
const BOWL_CX = (BOWL_LEFT_T + BOWL_RIGHT_T) / 2  // centre X = 313

export default function HeroCoupe() {
  const sectionRef    = useRef<HTMLElement>(null)
  const coupeRef      = useRef<HTMLDivElement>(null)
  const levelRef      = useRef<SVGRectElement>(null)   // le cache noir qui remonte = niveau liquide
  const surfaceRef    = useRef<SVGEllipseElement>(null) // ellipse à la surface du liquide
  const streamRef     = useRef<SVGRectElement>(null)    // filet de versement
  const splashRef     = useRef<SVGGElement>(null)       // splash officiel
  ...
  useGSAP(() => {
    ...
    // 2. Filet descend du header vers l'ouverture du bol
    tl.to(streamRef.current, {
      attr: { height: 305 },   // de y=-100 → atteint y=205 (bord du bol)
      duration: 0.7,
      ease: 'power2.in',
    }, 0.5)

    // 3. Liquide monte: le cache noir (levelRef) rétrécit de hauteur=BOWL_BOTTOM → BOWL_TOP
    //    → révèle le liquide de bas en haut dans le bol
    tl.to(levelRef.current, {
      attr: { height: BOWL_TOP },
      duration: 2.7,
      ease: 'power2.inOut',
    }, 1.0)

    // 4. Surface liquide monte en même temps
    ...
    // 6. Splash officiel — résultat = logo complet
  ...
  return (
    <section
      ref={sectionRef}
      className="hero-coupe-section"
    >
      <div className="container hero-coupe-grid">

        {/* ── Gauche : texte ────────────────────────────────────────── */}
        <div ref={textRef} style={{ opacity: r ? 1 : 0 }}>
          ...
          >
            ...
            <Link href="/reserver" className="btn btn-creme-on-dark btn-lg">RÉSERVER</Link>
          </div>
        </div>
        ...
        <div className="hero-coupe-right">
          ...
          >
            ...
            >
              <defs>
                {/* Gradient du liquide : orange vif en haut, rouge profond en bas */}
                <radialGradient id="hc-liquid" cx="50%" cy="30%" r="65%">
                  <stop offset="0%"   stopColor="rgba(255,130,65,.92)" />
                  <stop offset="40%"  stopColor="#E83B14" />
                  <stop offset="100%" stopColor="rgba(140,12,4,.88)" />
                </radialGradient>

                {/* Gradient du filet de versement */}
                <linearGradient id="hc-stream" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%"   stopColor="rgba(232,59,20,0)" />
                  <stop offset="50%"  stopColor="rgba(232,59,20,.55)" />
                  <stop offset="100%" stopColor="#E83B14" />
                </linearGradient>

                {/*
                  Mask pour le liquide :
                  1. Fond noir (rien visible)
                  2. Polygone du bol en blanc (zone visible = bowl intérieur seulement)
                  3. Cache noir qui REMONTE = le niveau du liquide
                     - height démarre à BOWL_BOTTOM = couvre tout le bol = verre vide
                     - height animé vers BOWL_TOP = couvre seulement au-dessus = verre plein
                */}
                <mask id="hc-liquid-mask">
                  <rect width="626.13" height="876" fill="black" />
                  <path
                    d={`M ${BOWL_LEFT_T} ${BOWL_TOP} L ${BOWL_RIGHT_T} ${BOWL_TOP} L ${BOWL_RIGHT_B} ${BOWL_BOTTOM} L ${BOWL_LEFT_B} ${BOWL_BOTTOM} Z`}
                    fill="white"
                  />
                  {/* Cache niveau — GSAP anime height de BOWL_BOTTOM à BOWL_TOP */}
                  <rect
                    ref={levelRef}
                    x="0"
                    y="0"
                    width="626.13"
                    height={r ? BOWL_TOP : BOWL_BOTTOM}
                    fill="black"
                  />
                </mask>

                {/* Mask bol uniquement pour la surface ellipse (sans le cache niveau) */}
                <mask id="hc-bowl-mask">
                  <rect width="626.13" height="876" fill="black" />
                  <path
                    d={`M ${BOWL_LEFT_T} ${BOWL_TOP} L ${BOWL_RIGHT_T} ${BOWL_TOP} L ${BOWL_RIGHT_B} ${BOWL_BOTTOM} L ${BOWL_LEFT_B} ${BOWL_BOTTOM} Z`}
                    fill="white"
              ...
              {/* Filet de versement — SVG overflow:visible le rend au-dessus du viewBox */}
              <rect
                ref={streamRef}
                x={BOWL_CX - 6}
                y={-100}
              ...
              {/* Structure du verre — couleurs originales, clairement visible sur crème */}
              <g opacity="0.75">
                {GLASS_PATHS.map((d, i) => (
                  <path key={`g${i}`} d={d} fill="#2b2b2b" />
                ))}
              </g>

              {/* Liquide — masqué au bol seulement, monte de bas en haut */}
              <rect
                x="0"
                y="0"
                width="626.13"
                height="876"
                fill="url(#hc-liquid)"
                mask="url(#hc-liquid-mask)"
              />

              {/* Surface liquide — ellipse à la limite du liquide, donne l'aspect concave */}
              <ellipse
                ref={surfaceRef}
                cx={BOWL_CX}
                cy={r ? BOWL_TOP + 8 : BOWL_BOTTOM}
                rx="148"
                ry="10"
                fill="rgba(255,140,65,0.72)"
                mask="url(#hc-bowl-mask)"
              />

              {/* Splash officiel — paths rouge du logo — opacity 0 → 1 à la fin */}
              <g
                ref={splashRef}
                opacity={r ? 1 : 0}
                fill="#E83B14"
              >
                {SPLASH_PATHS.map((d, i) => (
                  <path key={`s${i}`} d={d} />
                ))}
              </g>
            </svg>
      ...
      <div
        ref={scrollRef}
        className="hp-scroll-hint hero-coupe-scroll"
        style={{ opacity: r ? 1 : 0 }}
        aria-hidden="true"
      >
        <span>↓</span>
